...
window.EXAM_RULES = {
  ...
  ssc_hindi:  {name:'SSC CGL — Hindi',       time:10, lang:'hindi',   backspace:'full', highlight:'error',  minWPM:30, netFormula:'ssc',    info:['⏱ 10 Min','🎯 30 WPM','⌫ Backspace Allowed','📝 Net = Gross − (Errors ÷ Time)']},
  // SSC CHSL — same as CGL
  chsl_eng:   {name:'SSC CHSL — English',    time:10, lang:'english', backspace:'full', highlight:'error',  minWPM:35, netFormula:'ssc',    info:['⏱ 10 Min','🎯 35 WPM','⌫ Backspace Allowed','📝 Net = Gross − (Errors ÷ Time)']},
  chsl_hindi: {name:'SSC CHSL — Hindi',      time:10, lang:'hindi',   backspace:'full', highlight:'error',  minWPM:30, netFormula:'ssc',    info:['⏱ 10 Min','🎯 30 WPM','⌫ Backspace Allowed','📝 Net = Gross − (Errors ÷ Time)']},
...
// ── Admin Panel: single source of truth for view + nav switching ──────────
// Har admin section (Analytics / Passages / Schedule / Steno / Steno Videos)
// isi list mein hai. Naya section add karna ho toh sirf yahan add karo —
// baaki sab jagah automatically sahi se hide/show ho jaayega.
window.ADMIN_VIEW_IDS = ['admAnalyticsView','admPassagesView','admScheduleView','admStenoView','admStenoVideoView'];
window.ADMIN_NAV_IDS  = ['admNavAnalytics','admNavPassages','admNavSchedule','admNavSteno','admNavStenoVideo'];

window.hideAllAdminViews = function(){
  window.ADMIN_VIEW_IDS.forEach(id=>{
    const el=document.getElementById(id);
    if(el) el.style.display='none';
  });
  window.ADMIN_NAV_IDS.forEach(id=>{
    const el=document.getElementById(id);
    if(el) el.classList.remove('active');
  });
};

// viewId dikhao, navId ko active karo — baaki sab automatically hide ho jaate hain
window.showAdminView = function(viewId, navId){
  window.hideAllAdminViews();
