...
import { getAuth, signInWithEmailAndPassword, createUserWithEmailAndPassword, signOut, sendEmailVerification, sendPasswordResetEmail, onAuthStateChanged }
...
try {
  ...
  // Must match setGlobalOptions({region:'asia-south1'}) in functions/index.js —
  // a mismatch here means every httpsCallable() below fails with
  // functions/not-found, since the client would be asking the wrong region.
  fns   = getFunctions(fbApp, 'asia-south1');
  fbOk  = true;
...
function tabPath(tab){
  ...
  if(tab===6) return 'passages/normal/hindi_krutidev'; // KrutiDev 010 passages — alag array
...
function listenAll(){
  ...
  const paths=[
    ...
    {p:'passages/normal/hindi_krutidev',t:()=>window.PASS.hindi_krutidev}, // KrutiDev 010 alag
...
// ── Candidate-facing passage load (replaces listenAll() on student
//    pages) ─────────────────────────────────────────────────────
// FIX: listenAll() above reads 'passages/normal/*' directly, which used to
// be .read:true — every "PRO" passage's real text was fetchable by anyone,
// the lock in buildDd() (app.js) was UI-only. Students/guests now fetch
// those categories through the getPassages Cloud Function, which checks
// isPro server-side and blanks out locked passage text. 'live' passages and
// 'normal/speedmaster' were never Pro-gated in the UI, so they're kept as
// direct live listeners exactly as before — no behavior change there.
function loadPassagesForStudent(){
  if(!fbOk){ toast('❌ Firebase connect nahi ho paya.',false); return; }
  showLoader('Passages load ho rahe hain...');

  const fn = httpsCallable(fns, 'getPassages');
  fn().then((res)=>{
    const d = res.data || {};
    window.PASS.english        = d.english        || [];
    window.PASS.hindi          = d.hindi           || [];
    window.PASS.hindi_krutidev = d.hindi_krutidev  || [];
    window.PASS.numbers        = d.numbers         || [];
    hideLoader();
    if(typeof window.buildDd==='function') window.buildDd();
  }).catch(err=>{
    hideLoader();
    console.error('getPassages error', err);
    
    // Better error handling
    let errorMsg = '';
    if(err.code === 'functions/unavailable'){
      errorMsg = '⚠️ Cloud Functions not deployed. Admin se contact karein.';
    } else if(err.code === 'functions/not-found'){
      errorMsg = '⚠️ Passages function missing. System update ho raha hai.';
    } else if(err.code === 'functions/unauthenticated'){
      errorMsg = '⚠️ Login karke dobara try karein.';
    } else if(err.code === 'functions/permission-denied'){
      errorMsg = '❌ Access denied. Admin se contact karein.';
    } else if(err.message.includes('Failed to get document')){
      errorMsg = '⚠️ Database issue. 2 min mein try karein.';
    } else {
      errorMsg = '❌ Passages load nahi ho paye: ' + (err.message || 'Unknown error');
    }
    
    toast(errorMsg, false);
    
    // Fallback: try to load from Realtime Database directly
    if(typeof db !== 'undefined'){
      try{
        onValue(ref(db, 'passages/normal/english'), (snap)=>{
          if(snap.exists()){ 
            window.PASS.english = Object.values(snap.val() || {});
          }
        });
      } catch(e){
  ...
  livePaths.forEach(({p,t})=>{
    onValue(ref(db,p),(snap)=>{
      const arr=t(); arr.length=0;
      if(snap.exists()){ Object.keys(snap.val()).map(Number).sort((a,b)=>a-b).forEach(k=>arr.push(snap.val()[k])); }
      if(typeof window.buildDd==='function') window.buildDd();
    },(err)=>{ console.error('live passage listen error', err); });
  });
}
...
function saveTab(tab, retry=0){
  ...
  else if(tab===6) arr=window.PASS.hindi_krutidev; // KrutiDev 010
  ...
  return set(ref(db,path),obj)
    ...
    .catch(err=>{ hideLoader();
      if(retry<1){ return new Promise(res=>setTimeout(res,1500)).then(()=>saveTab(tab,retry+1)); }
...
// ── Admin Auth ───────────────────────────────────────────────
// This is the ONE admin account for the whole site — same email hardcoded
// in firebase-database-rules.json, functions/index.js, and
// reset-admin-password.js. BUG FOUND: signInWithEmailAndPassword() alone
// succeeds for *any* registered account (admin or not) — it just proves
// the email+password pair is valid, not that this person is the admin.
// Without an explicit check here, any signed-up user could open the Admin
// Panel UI with their own normal login credentials. The Realtime Database
// rules already block them from actually reading/writing real data (that's
// the "permission_denied" error a non-admin sees), but the panel's shell —
// nav tabs, "Admin: <their email>" label, etc. — would still incorrectly
// open for them. FIX: explicitly gate on the admin email here too.
...
window.fbAdminSignIn = function(email, password){
  ...
  return signInWithEmailAndPassword(auth, email, password)
    .then(cred=>{
      if(normalizedEmail !== ADMIN_EMAIL){
        return signOut(auth).then(()=>{
          const e = new Error('Ye account admin nahi hai — sirf registered admin email se login ho sakta hai.');
          e.code = 'not-admin';
          throw e;
        });
      }
    ...
    .catch(err=>{
      if(err.code !== 'not-admin') toast('❌ Login fail: '+err.message,false);
      throw err;
    });
};
...
// ── Defense-in-depth: keep watching auth state on the Admin Panel page
//    itself, not just at the login click. Covers an admin.html tab that
//    was ALREADY OPEN before this fix was deployed, or any future edge
//    case — if the signed-in account is ever not the admin, sign out and
//    reload straight back to the login gate.
...
//    candidates (see fbSendResetOTP further below) ──
...
// ── Forgot Password — Firebase's own reset-LINK email (no more custom
//    OTP codes / no more Cloud Function or Brevo SMTP involved for this).
//    Firebase emails the user a link that opens Firebase's own hosted
//    "set a new password" page — the app doesn't need an OTP-entry step
//    anymore, so fbVerifyResetOTP has been removed along with it. ────────
window.fbSendResetOTP = function(email){
  if(!fbOk) return Promise.reject(new Error('Firebase ready nahi hai, page refresh karein.'));
  return sendPasswordResetEmail(auth, email)
    .then(()=> ({ success: true }))
    .catch(err=>{
      let msg = 'Reset link bhejne mein error aaya.';
      if(err.code === 'auth/user-not-found') msg = 'Ye email registered nahi hai.';
      else if(err.code === 'auth/invalid-email') msg = 'Email format sahi nahi hai.';
      else if(err.code === 'auth/too-many-requests') msg = 'Bahut zyada attempts ho gaye, thodi der baad try karein.';
      throw new Error(msg);
    });
};

// Turns a raw Firebase callable-function error into a friendly Hinglish
// message. Most functions already throw a proper HttpsError with a human
// message — but if one crashes with an unhandled exception, the SDK
// replaces it with a bare code like 'internal' and a message that's just
// that same word, which is meaningless to a user. Safety net for that case.
function friendlyFnError(err, fallback){
  const code = err && err.code;
  const msg  = (err && err.message || '').trim().toLowerCase();
  if(code === 'functions/internal' || code === 'internal' || msg === 'internal'){
    return 'Kuch technical dikkat aa rahi hai. Thodi der baad dobara try karein.';
  }
  if(code === 'functions/unavailable' || code === 'unavailable'){
    return 'Service abhi available nahi hai. Internet check karein aur dobara try karein.';
  }
...
window.fbSendContactMessage = function(name, email, subject, message){
  ...
  return fn({ name, email, subject, message }).then(r=>r.data)
    .catch(err=>{ throw new Error(friendlyFnError(err, 'Message bhejne mein error aaya.')); });
};
...
//
// FIX: this used to read 'stenoVideos' directly from the database, which
// was .read:true — every "PRO" video's real youtubeId was fetchable by
// anyone, the lock was UI-only. It now goes through the getStenoVideos
// Cloud Function, which checks isPro server-side and blanks out youtubeId
// for anything locked. Same return shape as before, so nothing else in
// candidate.html needs to change.
window.fbGetStenoVideosOnce = function(){
  if(!fbOk) return Promise.resolve(null);
  const fn = httpsCallable(fns, 'getStenoVideos');
  return fn().then(res => (res.data && res.data.videos) ? res.data.videos : null)
    .catch(err=>{ console.error('getStenoVideos error', err); return null; });
};
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  // admGate only exists in admin.html — everywhere else (candidate.html,
  // typing.html, etc.) is a student/guest page, which must go through
  // getPassages() so Pro-locked passage text is never sent to the browser.
  if(document.getElementById('admGate')){
    listenAll();
  } else {
    loadPassagesForStudent();
  }
